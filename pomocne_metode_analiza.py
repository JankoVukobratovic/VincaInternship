...
def render_comparisons(strips: list[np.ndarray[Any, np.dtype[np.float64]]], element_keys, element_maps: list[ dict[str, dict[str, str | float]]], strip_names: list[str], figname, savename):
    ...
    for row_idx, key in enumerate(element_keys):
        ...
        for col_idx in range(num_scans):
            ...
            if col_idx ==  2:
                abs_max = np.nanmax(np.abs(img_data))
                abs_max = max(abs_max, 0.00001)
